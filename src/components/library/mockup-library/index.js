...
const MockupLibrary = () => {
  ...
  const [cupImages, setCupImages] = useState([]);
  const [frameImages, setFrameImages] = useState([]);
  const [bagImages, setBagImages] = useState([]);
  const [pillowImages, setPillowImages] = useState([]);
  ...
  useEffect(() => {
    fetch("http://localhost/svgheart/wp-json/mockup/v1/get-mockup")
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    if (value === "") {
      console.log(apiData);
      apiData
        .filter((data) => data.category === "t-shirt")
        .map((tshirtData) => {
          return tshirt.push(tshirtData.mockup);
        });

      apiData
        .filter((data) => data.category === "bag")
        .map((bagData) => bag.push(bagData.mockup));

      apiData
        .filter((data) => data.category === "mug")
        .map((mugData) => mug.push(mugData.mockup));

      apiData
        .filter((data) => data.category === "frame")
        .map((frameData) => frame.push(frameData.mockup));

      apiData
        .filter((data) => data.category === "pillow")
        .map((pillowData) => pillow.push(pillowData.mockup));

      setTshirtImages(tshirt);
      setCupImages(mug);
      setFrameImages(frame);
      setBagImages(bag);
      setPillowImages(pillow);
    }
  }, [apiData, value]);

  useEffect(() => {
    // Function to filter objects based on input value
    function filterObjects(inputValue) {
      // Convert input value to lowercase for case-insensitive matching
      const searchTerm = inputValue.toLowerCase();

      // Filter objects based on matching tags
      const filteredResults = apiData.filter((obj) => {
        // Check if any tag contains the search term
        return obj.tags.some((tag) => tag.toLowerCase().includes(searchTerm));
      });
    ...
    // Example usage:
    const filteredResults = filterObjects(searchValue);
  ...
  useEffect(() => {
    ...
    // console.log(searchValue, "searchV");
  ...
  //   // apiData.map((data) => console.log(data.tags));
  // };
